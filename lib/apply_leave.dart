...
class apply_leave extends StatefulWidget {
  ...
  const apply_leave({super.key, required this.buttonName, required this.lookupDetailId, required this.employename});
...
class _apply_leaveeState extends State<apply_leave> {
  ...
  final TextEditingController _leavetext = TextEditingController();
  ...
  List<Map<String, dynamic>> otherLeavesList = []; // Create a list for other leave types
  ...
  Future<void> _selectDate(bool isTodayHoliday) async {
    ...
    if (widget.buttonName == "test" && selectedleaveValue == -1) {
      ...
      Commonutils.showCustomToastMessageLong("Please Select Leave Type To Select From Date", context, 1, 5);
    } else {
      if (widget.buttonName == "CL" || selectedleaveName == "CL") {
        ...
        DateTime endDate = initialDate.add(const Duration(days: 7));
        // DateTime tempEndDate = endDate; // Store the original end date
  ...
  bool _isHoliday(DateTime date) {
    ...
    for (final holiday in holidayList) {
      if (holiday.fromDate.year == date.year && holiday.fromDate.month == date.month && holiday.fromDate.day == date.day) {
  ...
  bool selectableDayPredicate(DateTime date) {
    ...
    final isHoliday = holidayList.any((holiday) {
      ...
      return date.isAfter(holidayFromDate.subtract(const Duration(days: 1))) && date.isBefore(holidayToDate.add(const Duration(days: 1)));
    });

    // Return true if it's not Saturday, not Sunday, not a holiday, and not before today
    return !saturday && !sunday && !isHoliday && date.isAfter(DateTime.now().subtract(const Duration(days: 1)));
  }
  ...
  Future<void> _selectToDate() async {
    ...
    DateTime initialDate = selectedToDate ?? selectedDate!;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData.containsKey('mininumDaysToConsiderAsLongLeave')) {
          final minimumDaysToConsiderAsLongLeave = responseData['mininumDaysToConsiderAsLongLeave'];
          if (minimumDaysToConsiderAsLongLeave is int) {
            print('Minimum Days to Consider as Long Leave: $minimumDaysToConsiderAsLongLeave');
            setState(() {
              DaysToConsiderAsLongLeave = minimumDaysToConsiderAsLongLeave;
              print('DaysToConsiderLongLeave: $DaysToConsiderAsLongLeave');
            });
          } else {
            print('Error: Value for mininumDaysToConsiderAsLongLeave is not an integer');
          }
        } else {
  ...
  void _loadleaveslist(String empolyeeId) async {
    ...
    try {
      final url = Uri.parse('$baseUrl$getleavesapi$empolyeeId/$currentYear');
      ...
      if (response.statusCode == 200) {
        ...
        otherLeavesList.clear(); // Clear the WFH list before adding new entries
        ...
        for (var leave in data) {
          ...
          } else if ((leave['status'] == 'Accepted' || leave['status'] == 'Approved' || leave['status'] == 'Pending') &&
              (leave['isDeleted'] == false || leave['isDeleted'] == null)) {
            otherLeavesList
                .add({'fromDate': leave['fromDate'], 'toDate': leave['toDate'], 'leaveType': leave['leaveType'], 'status': leave['status']});
          }
        }

        // Print all "WFH" from dates and to dates
        for (var wfhEntry in wfhLeavesList) {
        ...
        for (var otherLeave in otherLeavesList) {
          print('${otherLeave['leaveType']} From Date: ${otherLeave['fromDate']}');
          print('${otherLeave['leaveType']} To Date: ${otherLeave['toDate']}');
        }

        print('API Response WFH Leaves List length: ${wfhLeavesList.length}');
        print('API Response Other Leaves List length: ${otherLeavesList.length}');
      } else {
        Commonutils.showCustomToastMessageLong('Error: ${response.body}', context, 1, 4);
        print('Error: ${response.statusCode} - ${response.reasonPhrase}');
      }
...
//Working code for wfh restriction, commented to add other leave types by Arun
  ...
  Future<void> applyleave() async {
    ...
    if (widget.buttonName == "PL" || selectedleaveName == "PL") {
      if (isChecked == false) {
        ...
        int selectedDays = selectedToDatePL.difference(selectedDate!).inDays + 1;
        print('selectedDays:$selectedDays');
      ...
      bool isMatchFound = wfhLeavesList.any((wfhEntry) {
        ...
        if (wfhFromDateStr != null && wfhToDateStr != null) {
          ...
          if (wfhFromDate != null && wfhToDate != null) {
            ...
            bool isMatch = (selectedDate!.isBefore(wfhFromDate) && selectedToDateLeaves!.isAfter(wfhFromDate)) || // Overlap at the beginning
                (selectedDate!.isBefore(wfhToDate) && selectedToDateLeaves!.isAfter(wfhToDate)) || // Overlap at the end
                (selectedDate!.isAtSameMomentAs(wfhFromDate) || selectedToDateLeaves!.isAtSameMomentAs(wfhToDate)) || // Exact match
                (selectedDate!.isAfter(wfhFromDate) && selectedToDateLeaves.isBefore(wfhToDate)) || // Other leave completely inside selected dates
                (wfhFromDate.isAfter(selectedDate!) && wfhToDate.isBefore(selectedToDateLeaves)); // Other leave in between selected dates
        ...
        if (isMatchFound) {
          ...
          bool isOtherLeaveFound = otherLeavesList.any((otherLeave) {
            ...
            if (otherFromDateStr != null && otherToDateStr != null) {
              ...
              DateTime? otherFromDate = DateTime.tryParse(otherFromDateStr.substring(0, 10)); // Extracting date part
              DateTime? otherToDate = DateTime.tryParse(otherToDateStr.substring(0, 10)); // Extracting date part
              ...
              if (otherFromDate != null && otherToDate != null) {
                ...
                DateTime selectedFromDate = DateTime(selectedDate!.year, selectedDate!.month, selectedDate!.day);
                DateTime selectedToDatee = DateTime(selectedToDateLeaves!.year, selectedToDateLeaves.month, selectedToDateLeaves.day);
                ...
                bool isOverlap = (selectedDate!.isBefore(otherFromDate) &&
                        selectedToDateLeaves.isAfter(otherFromDate)) || // Overlap at the beginning
                    (selectedDate!.isBefore(otherToDate) && selectedToDateLeaves.isAfter(otherToDate)) || // Overlap at the end
                    (selectedDate!.isAtSameMomentAs(otherFromDate) || selectedToDateLeaves.isAtSameMomentAs(otherToDate)) || // Exact match
                    (selectedDate!.isAfter(otherFromDate) &&
                        selectedToDateLeaves.isBefore(otherToDate)) || // Other leave completely inside selected dates
                    (otherFromDate.isAfter(selectedDate!) && otherToDate.isBefore(selectedToDateLeaves)); // Other leave in between selected dates
          ...
          if (isOtherLeaveFound) {
            ...
            if (isValid) {
              ...
              //     : "";
              String formattedFromDate = DateFormat('dd MMM yyyy').format(DateTime.parse(overlappingFromDate.toString()));
              String formattedToDate = DateFormat('dd MMM yyyy').format(DateTime.parse(overlappingToDate.toString()));

              Commonutils.showCustomToastMessageLong(
                  "Ooops !!! Duplicate leave is existing from $formattedFromDate to $formattedToDate dates", context, 1, 5);
              isValid = false;
              hasValidationFailed = true;
              fromDateSelected = false;
              _fromdateController.clear();
  ...
  void _sendLeaveRequest(bool confirmedToSplitWFH, String fromdate, String todate) async {
    bool isConnected = await Commonutils.checkInternetConnectivity();
    if (isConnected) {
      print('Connected to the internet');
    } else {
                ...
                child: Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      if (widget.buttonName == "test")
                        Padding(
                          ...
                          child: Container(
                            width: MediaQuery.of(context).size.width,
                            decoration: BoxDecoration(
                              border: Border.all(color: const Color(0xFFf15f22), width: 1.5),
                              borderRadius: BorderRadius.circular(5.0),
                                    ...
                                    : DropdownButton<int>(
                                        ...
                                        onChanged: (value) {
                                          setState(() {
                                            ...
                                            selectedDate = null;
                                            selectedValue = 0;
                                            print('_onchangedfromdateController==$_fromdateController');
                                            print('_onchangedtodateController==$_todateController');
                                            print('onchangedselectedToDate==$selectedToDate');
                                            ...
                                            if (value == -1) {
                                              ...
                                              setState(() {
                                                ...
                                                print('_onselectedfromdateController==$_fromdateController');
                                                print('_onselectedtodateController==$_todateController');
                                                print('onselectedselectedToDate==$selectedToDate');
                                              });
                                        ...
                                        items: [
                                          ...
                                          for (LookupDetail item in lookupDetails)
                                            if (['WFH', 'LWP', 'LL'].contains(item.name) ||
                                                (item.name == 'CL' && availablecls != 0.0) ||
                                                (item.name == 'PL' && availablepls != 0.0))
                                              DropdownMenuItem<int>(
                      ...
                      if (widget.buttonName == "CL" || widget.buttonName == "PL" || selectedleaveName == "CL" || selectedleaveName == "PL")
                        Padding(
                          padding: const EdgeInsets.only(left: 10, top: 10.0, right: 0),
                          child: Row(
                            children: [
                              const Text(
                                'Is Halfday Leave?',
                                  ...
                                  child: TextFormField(
                                    ...
                                    decoration: InputDecoration(
                                      ...
                                      fillColor: !isChecked && fromDateSelected
                                          ? Colors.white
                                          : Colors.grey[300], // Add this line to change background color
                                    ),
                                  ),
                                ),
                              ),
                                    ...
                                    setState(() {
                                      ...
                                      if (value.length > 256) {
                                        ...
                                        _leavetext.value = TextEditingValue(
                                          ...
                                          selection: const TextSelection.collapsed(offset: 256),
                                        );
                                      }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (final holiday in holidayList) {
          ...
          for (DateTime date = holidayFromDate; date.isBefore(holidayToDate.add(const Duration(days: 1))); date = date.add(const Duration(days: 1))) {
            // Check if today's date falls within the holiday's date range
            String holidayDate = DateFormat("yyyy-MM-dd'T'HH:mm:ss").format(date);
            if (formattedDate == holidayDate) {
              isTodayHoliday = true;
  ...
  Future<void> _selectToCLDate() async {
    ...
    while (_isHolidayOrWeekend(initialDate) || !selectableDayPredicate(initialDate)) {
      initialDate = initialDate.add(const Duration(days: 1));
    }
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Column(
            ...
            children: [
              Text('Your Leave Request is within the WFH Span. will you want to split this, Please confirm this by "Clicking" Confirm else "Cancel"')
            ],
          ),
          actions: [
            ElevatedButton(
